...
window.onload = function () {
    ...
    var lImage = document.querySelector("#outputImages");
    lImage.innerHTML += buildImages(images);

};

buildImages = function(obj) {
    var strImg = "";

    for (var j = 0; j < obj.length; j++) {
        strImg += "<figure>";
        strImg += "<img src='" + obj[j]["url"] + "' alt='"+ obj[j]["alt"] +"'>";
        strImg += "<figcaption>" + obj[j]["caption"] + "</figcaption>";
        strImg += "</figure>";
    }
    return strImg;
};